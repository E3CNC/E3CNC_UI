...
properties = {
  ...
  yOffset: 0,                          // Offset to add to Y coordinates (for machines where CAM Y ≠ machine Y)
  gcode_end: "",                       // end gcode
...
propertyDefinitions = {
  ...
  yOffset : {
    title: "Y Offset", description: "Value added to all Y coords (e.g. 100 shifts CAM Y-100→machine Y0)", group: "4 Gcode",
    type: "float", default_mm: 0.0, default_in: 0.0
  },
...
function onSection() {
  ...
  WriteComment(eComment.Info, "   Y Min: " + xyzFormat.format(currentSection.getGlobalRange(vectorY).getMinimum() + properties.yOffset) + " - Y Max: " + xyzFormat.format(currentSection.getGlobalRange(vectorY).getMaximum() + properties.yOffset));
  ...
  WriteBlock(
    ...
    , "Y=" + xyzFormat.format(currentSection.getGlobalRange(vectorY).getMinimum() + properties.yOffset) + "," + xyzFormat.format(currentSection.getGlobalRange(vectorY).getMaximum() + properties.yOffset)
...
function onRapid(x, y, z) {
  ...
  rapidMovements(x, y + properties.yOffset, z);
}
